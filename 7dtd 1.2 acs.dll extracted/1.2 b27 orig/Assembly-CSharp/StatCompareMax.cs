...
public class StatCompareMax : StatCompareCurrent
{
	public override bool IsValid(MinEventParams _params)
	{
		...
		case StatCompareCurrent.StatTypes.Health:
			if (!this.invert)
			{
				return RequirementBase.compareValues(this.target.Stats.Health.Max, this.operation, this.value);
			}
			return !RequirementBase.compareValues(this.target.Stats.Health.Max, this.operation, this.value);
		case StatCompareCurrent.StatTypes.Stamina:
			if (!this.invert)
			{
				return RequirementBase.compareValues(this.target.Stats.Stamina.Max, this.operation, this.value);
			}
			return !RequirementBase.compareValues(this.target.Stats.Stamina.Max, this.operation, this.value);
		case StatCompareCurrent.StatTypes.Food:
			if (!this.invert)
			{
				return RequirementBase.compareValues(this.target.Stats.Food.Max, this.operation, this.value);
			}
			return !RequirementBase.compareValues(this.target.Stats.Food.Max, this.operation, this.value);
		case StatCompareCurrent.StatTypes.Water:
			if (!this.invert)
			{
				return RequirementBase.compareValues(this.target.Stats.Water.Max, this.operation, this.value);
			}
			return !RequirementBase.compareValues(this.target.Stats.Water.Max, this.operation, this.value);
		default:
			return false;
